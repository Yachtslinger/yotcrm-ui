...
function parseRUYachts(url: string, html: string): VesselData {
  ...
  $("img[src], img[data-src]").each((_, img) => {
    ...
    if (!src) return;
    if (/logo|icon|sprite|no-photo|placeholder|\.svg|1x1|pixel|persons?\/|broker|agent|staff|team|farzan|avatar|\/th\/th-|model\/cover|shipyard\/logo|events\/|yacht-news\//i.test(src)) return;
