  ...
  try {
    ...
    return NextResponse.json({
      ...
      vessel: {
        ...
        crew: vessel.crew,
        staterooms: (vessel as any).staterooms,
        cabins: (vessel as any).cabins,
        grossTonnage: (vessel as any).grossTonnage,
        price: (vessel as any).price || (vessel as any).askingPrice,
        location: (vessel as any).location,
        description: (vessel as any).description,
        images: (vessel.images || []).map((i: any) => i?.src || i).filter(Boolean).slice(0, 60),
