...
const PROMPTS: Record<string, (c: Record<string,string>) => string> = {
  executiveSummary:      c => `You are a yacht broker. Write a 3-4 sentence Executive Summary for a market analysis of a ${c.year} ${c.make} ${c.model} named "${c.vessel}", ${c.length}, asking ${c.askingPrice}. Market: ${c.soldCount} sold comps avg ${c.avgSold}, ${c.activeCount} active avg ${c.avgActive}, avg DOM ${c.avgDom} days.${c.notes ? ` Notes: ${c.notes}` : ""} First-person broker voice. No AI references. Return only the paragraph.`,
  ...
  competitivePositioning:c => `You are a yacht broker. Write a 4-5 sentence Competitive Positioning paragraph. Vessel: ${c.year} ${c.make} ${c.model} "${c.vessel}", ${c.length}, asking ${c.askingPrice}.${c.notes ? ` Condition: ${c.notes}` : ""} Competitors: ${c.activeTable}. First-person broker voice. No AI references. Return only the paragraph.`,
  priceReductionStrategy:c => `You are a yacht broker. Write a 3-4 sentence Price Reduction Strategy. Vessel: ${c.year} ${c.make} ${c.model}, recommended ask ${c.recommendedPrice}. Avg DOM: ${c.avgDom} days. ${c.activeCount} active competitors. Give specific reduction triggers with percentages and timing. First-person broker voice. No AI references. Return only the paragraph.`,
  ...
  pricingRationale:      c => `You are a yacht broker. Write a 3-4 sentence Pricing Rationale. Vessel: ${c.year} ${c.make} ${c.model} "${c.vessel}", ${c.length}. Recommended: ${c.recommendedPrice}. Ask: ${c.askingPrice}. Comps: ${c.soldTable}. Justify the price using comp data. First-person broker voice. No AI references. Return only the paragraph.`,
  domRationale:          c => `You are a yacht broker. Write a 2-3 sentence DOM Forecast rationale. Vessel: ${c.year} ${c.make} ${c.model}. Best case: ${c.domLow} days. Expected: ${c.domHigh} days. Avg comp DOM: ${c.avgDom} days. ${c.activeCount} active competitors. First-person broker voice. No AI references. Return only the paragraph.`,
  keyDifferentiators:    c => `You are a yacht broker. List 4-6 key selling points for a ${c.year} ${c.make} ${c.model} "${c.vessel}", ${c.length}, asking ${c.askingPrice}.${c.notes ? ` Features: ${c.notes}` : ""} Return ONLY a JSON array of strings like ["Point one","Point two"]. No other text.`,
};
