...
async function sync() {
  ...
  // Match todos are Railway-side operational state — regenerated by reruns, never synced from local.
  // Only sync non-match todos (manual tasks, calendar items, reminders).
  let todos = []; try { todos = db.prepare("SELECT * FROM todos WHERE todo_type != 'match' OR todo_type IS NULL").all(); } catch {}
